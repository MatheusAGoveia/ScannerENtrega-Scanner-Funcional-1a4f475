...
def test_extract_or_build_cpe():
    assert extract_or_build_cpe("OpenSSH sshd", "8.2") == "cpe:2.3:a:openbsd:openssh:8.2:*:*:*:*:*:*:*"
    assert extract_or_build_cpe("Microsoft IIS httpd", None) == "cpe:2.3:a:microsoft:internet_information_services:*:*:*:*:*:*:*:*"
    assert extract_or_build_cpe("unknown", "1.0") is None

def test_normalize_service_observation():
    data = normalize_service_observation("ssl/http", "Microsoft IIS httpd", "10.0")
    assert data.service_name_normalized == "https"
    assert data.product_normalized == "iis"
    assert data.version_normalized == "10.0"
    assert data.cpe == "cpe:2.3:a:microsoft:internet_information_services:10.0:*:*:*:*:*:*:*"
